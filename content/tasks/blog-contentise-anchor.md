...
|---|---|---|
| 111-126 | Brand link in the header (`href={localiseLink("/")}`, `Heading as="span"` child) | `link` block (legacy alias → `anchor`, registry :436) — header shell work, see parent task |
| 306-310 | Author avatar href → `/blog/by-author/${post.author \|\| "Artefact Team"}` | `anchor` block wrapping the `avatar` block; href bound |
| 323-331 | Author name link → `/blog/by-author/...` | `anchor` block, `text` child |

(Raw `<a>` elements — back link :168, related-post cards :473, footer :534 — are not the `Anchor` component; they're covered by the `anchor`-block migration of back/related/footer chrome, not this subtask.)
...
- `Anchor` no longer imported in `blog/[slug].tsx`; author + brand links render from `anchor` blocks in the template with bound hrefs.
