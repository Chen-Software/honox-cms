...
This component is available as a `dialog` block in the [Page Builder](/docs/PageBuilder) (`content/pages/*.json`). `trigger` is a nested block list (the CMS always submits an array), and so is `footer` — a row of blocks (typically Buttons) rendered alongside the dedicated Confirm/Cancel shortcuts, for actions that don't fit `confirmText`/`cancelText` (e.g. a "Refresh" button dispatching a custom event):
...
```json
{
  ...
  "footer": [{ "type": "button", "variant": "outline", "text": "Refresh" }]
}
```
...
| `footer` | \`string \ | JSX.Element\` | Custom footer content. In CMS content this field is a block list, rendered as a fragment. |
