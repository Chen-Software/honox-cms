...
This component is available as a `drawer` block in the [Page Builder](/docs/PageBuilder) (`content/pages/*.json`). `trigger` is a nested block list (the CMS always submits an array), and so is `footer` — a row of blocks (typically Buttons) rendered alongside the dedicated Confirm/Cancel shortcuts:
...
```json
{
  ...
  "footer": [{ "type": "button", "variant": "outline", "text": "Reset" }]
}
```
...
| `footer` | \`string \ | JSX.Element\` | Custom footer content. In CMS content this field is a block list, rendered as a fragment. For `start`/`end` placements the footer is pinned to the bottom of the panel regardless of body length. |
