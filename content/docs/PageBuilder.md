...
* `resolveType()` first runs the type through a `TYPE_ALIASES` table (e.g. `"link"` → `anchor` — a legacy alias; `anchor` is canonical — plus `"hover-card"` → `hoverCard`, `"menu"` → `dropdown`), so CMS content and component names can drift slightly without breaking.
